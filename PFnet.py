...
class PFnetClass(object):
    ...
    def build(self,map_data,ins,init_particle_states,labels,is_training):
        self.outputs = self.buildRnn(map_data,ins,init_particle_states,self._is_first_step)
    ...
    def buildRnn(self,map_data,ins,init_particle_states,is_first_step):
        batch_size, particle_nums = init_particle_states.get_shape().as_list()[:2]        
        map_shape = map_data.shape.as_list()[-1]
        
        init_particle_weights = tf.constant(np.log(1.0/float(particle_nums)),
                        shape=(batch_size,particle_nums),dtype=tf.float32)
