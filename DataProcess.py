...
class LabelData():
    ...
    def getData(self,num_epochs = 1):
        data = np.zeros((1,6))
        for file in self._file_list:
            content = np.loadtxt(file,dtype = np.float,delimiter = ',')
            length = len(content)
        ...
        features = data[:,0:2]
        labels = data[:,3:5]
        dataset = tf.data.Dataset.from_tensor_slices((features,labels)).batch(self._time_step)
        return dataset
#-----------------------------------------------------------------------------"        
"""
...
class MapData:
    ...
    def getMap(self):
        ...
        self._image = tf.image.decode_image(file)
        return self._image
